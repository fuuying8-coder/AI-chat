...
const checkConnection = async () => {
  ...
  backendStatus.value.connected = result.connected
  backendStatus.value.checking = false
  if (result.connected) {
    backendStatus.value.message = '已连接'
  } else {
    backendStatus.value.message = '未连接'
  }
}
...
<template>
  <el-drawer v-model="visible" title="设置" direction="rtl" size="350px">
    <div class="setting-container">
      <!-- 后端连接状态 -->
      <div class="setting-item backend-status">
        <div class="setting-label-row">
          <div class="label-with-tooltip">
            <span>后端服务</span>
            <el-tooltip 
              :content="backendStatus.connected ? '后端服务运行正常' : '后端服务未运行，将使用直接 API 调用'"
              placement="top"
            >
        ...
        <div v-if="!backendStatus.connected" class="backend-tip">
          <el-text size="small" type="info">
            提示：运行 <code>cd server && npm run dev</code> 启动后端服务
          </el-text>
        </div>
      </div>
